...
TSharedPtr<FJsonObject> FUnrealMCPEditorCommands::HandleSpawnActor(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (NewActor)
    {
        ...
        // If this is a StaticMeshActor and asset_path is provided, set the mesh
        if (ActorType == TEXT("StaticMeshActor") && Params->HasField(TEXT("asset_path")))
        {
            ...
            if (MeshActor && MeshActor->GetStaticMeshComponent())
            {
                ...
                if (Mesh)
                {
                    ...
                    UE_LOG(LogTemp, Display, TEXT("Set static mesh for actor '%s' to '%s'"), *ActorName, *AssetPath);
                }
                else
                {
...
{
    // Get optional path parameter (default to /Game)
    ...
    // Get optional filter pattern
    FString FilterPattern = TEXT("");
    if (Params->HasField(TEXT("filter")))
    {
        FilterPattern = Params->GetStringField(TEXT("filter"));
    }
    ...
    // Use AssetRegistry to list assets
    ...
    if (bRecursive)
    {
        AssetRegistry.GetAssetsByPath(*SearchPath, AssetList, true);
    }
    else
    {
        AssetRegistry.GetAssetsByPath(*SearchPath, AssetList, false);
    }

    // Build the result
    TArray<TSharedPtr<FJsonValue>> AssetsArray;
    TMap<FString, TArray<TSharedPtr<FJsonValue>>> FolderStructure;
    ...
    for (const FAssetData& Asset : AssetList)
    {
        ...
        if (!FilterPattern.IsEmpty() && !AssetName.Contains(FilterPattern))
        {
            continue;
        }
...
TSharedPtr<FJsonObject> FUnrealMCPEditorCommands::HandleGetAssetDetails(const TSharedPtr<FJsonObject>& Params)
{
    ...
    ResultObj->SetStringField(TEXT("class"), AssetData.AssetClassPath.ToString());

    // If it's a Blueprint, get more details
...
TSharedPtr<FJsonObject> FUnrealMCPEditorCommands::HandleGetProjectInfo(const TSharedPtr<FJsonObject>& Params)
{
    TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
    ...
    ResultObj->SetStringField(TEXT("engine_version"), FString::Printf(TEXT("%d.%d.%d"),
        ENGINE_MAJOR_VERSION, ENGINE_MINOR_VERSION, ENGINE_PATCH_VERSION));

    // Get current level name
    if (GWorld)
    {
        ResultObj->SetStringField(TEXT("current_level"), GWorld->GetName());
    }

    // Get content folder path
    ResultObj->SetStringField(TEXT("content_path"), FPaths::ProjectContentDir());
